...
class MetadataGroupStage(GroupStage):
    ...
    def process(self, groups_list):
        ...
        prev_camera = None
        prev_date = None
        for image_path in self.config.get_image_list():
            station, camera, date, time = self.extract_camera_trap_info(image_path)
            hour, minute, sec = self.extract_time_info(time)
            ...
            if ((station == prev_station) and (camera == prev_camera) and (date == prev_date) and (
                ...
                grouped_images.append(self.filename_without_ext(image_path))
    ...
    def extract_camera_trap_info(self, image_path):
        logging.info("Extracting camera trap info")
        filename = self.filename_without_ext(image_path)
    ...
    def extract_time_info(self, time):
